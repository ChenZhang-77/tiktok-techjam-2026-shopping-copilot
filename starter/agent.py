...
from starter.core.clarification import select_clarification
...
class Agent:
    ...
    def respond(
        ...
        if state is not None:
            ...
            question_selection = select_clarification(
                state,
                turn=turn,
                candidate_texts=candidate_texts,
                decision_evidence=decision_evidence,
            )
            diagnostics["question_selection"] = question_selection.to_diagnostics()
            if question_selection.ask_attribute:
                response["ask_attribute"] = question_selection.ask_attribute
                base_message = response.get("message") if isinstance(response, dict) else ""
                response["message"] = (
                    f"{base_message} {question_selection.question}".strip()
                )
