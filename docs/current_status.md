...
This checkout is `llm`, the optional Plan Two publication branch. Its default remains local/no-external-LLM; merely checking out this branch does not turn on an API.
...
Evidence: [F2 decision](b10b_paired_verification.md) and [bound results](b10b_paired_verification_result.json).
...
A14-0/1 parity-only interface and evidence infrastructure remains in this branch; it does not activate the experimental selector. The annotation bundle and valid-34 data remain historical diagnostics, not gold.
